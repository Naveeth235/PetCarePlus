  ...
  if (isLoading) {
    return (
      <div className="flex h-64 items-center justify-center">
        <div className="text-lg text-slate-600">Loading your appointments...</div>
      </div>
    );
  }
          ...
          >
            ...
            {tab.key !== 'all' && (
              <span className="ml-2 inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-gray-600 bg-gray-200 rounded-full">
                {appointments.filter(a => a.status.toLowerCase() === tab.key).length}
              </span>
            )}
          </button>
        ))}
      ...
      {filteredAppointments.length > 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                    <div>
                      <p className="text-sm text-gray-600">
                        <span className="font-medium">Date:</span> {new Date(appointment.requestedDateTime).toLocaleDateString()}
                      </p>
                      <p className="text-sm text-gray-600">
                        <span className="font-medium">Time:</span> {new Date(appointment.requestedDateTime).toLocaleTimeString()}
                      </p>
                      {appointment.vetName && (
                        <p className="text-sm text-gray-600">
                          <span className="font-medium">Veterinarian:</span> {appointment.vetName}
                        </p>
                      )}
                    </div>
                    <div>
        ...
        <div className="rounded-2xl bg-white p-8 text-center ring-1 ring-slate-200">
          ...
          <p className="text-slate-500">
            {filter === 'all' && "Request your first appointment to get started."}
          </p>
        </div>
      )}
    </div>
